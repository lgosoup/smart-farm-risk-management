...
def calculate_over_control_penalty(
    ...
    if (
        float(current_inputs.get("water_temperature", 99.0))
        <= float(config["low_temperature_cooler_penalty_threshold"])
        and float(control["cooler_minutes"]) > 0.0
    ):
        penalty += float(control["cooler_minutes"]) / max(candidate_max_value("cooler_minutes", config), 1.0)

    if (
        float(current_inputs.get("flow_ratio", 0.0))
        >= float(config["normal_fr_circulation_penalty_threshold"])
...
def calculate_device_energy_breakdown(control: Dict[str, float], config: Dict[str, object]) -> Dict[str, float]:
    ...
    peltier_watt = float(config["estimated_peltier_1_watt"]) + float(config["estimated_peltier_2_watt"])
    cooling_watt = float(config["estimated_cooling_pump_watt"]) + float(config["estimated_cooling_fan_watt"])
    return {
        "main_water_pump": float(config["estimated_main_water_pump_watt"]) * float(control["circulation_pump_minutes"]) / 60.0,
        "nutrient_pump": float(config["estimated_nutrient_pump_watt"]) * nutrient_minutes / 60.0,
        "oxygen_pump": float(config["estimated_oxygen_pump_watt"]) * float(control["aeration_minutes"]) / 60.0,
        "peltier": peltier_watt * float(control["cooler_minutes"]) / 60.0,
        "cooling_pump_fan": cooling_watt * float(control["fan_minutes"]) / 60.0,
        "led_bar": float(config["estimated_led_bar_watt"]) * float(control["led_minutes"]) / 60.0,
    }
